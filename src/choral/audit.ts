...
import {
  ...
  MusicXmlPart,
  MusicXmlScore,
  PlannedNoteCorrection,
  SingingProjectSnapshot,
  SnapshotNote
} from "./types.js";
import {
  ecclesiasticalLatinWord,
  formatDirectPhonemes,
  normalizePhonemeString,
  splitSyllablePhonemes
} from "./latin.js";
...
function streamKey(note: MusicXmlNote): string {
  return `${note.staff}:${note.voice}`;
}

function groupWords(notes: MusicXmlNote[]): Array<{ events: Array<{ noteIndex: number; note: MusicXmlNote }>; endIndex: number }> {
  const lyricEvents = notes
    .map((note, noteIndex) => ({ noteIndex, note }))
    .filter((entry) => entry.note.lyric?.text);
  const words: Array<{ events: Array<{ noteIndex: number; note: MusicXmlNote }>; endIndex: number }> = [];
  let current: Array<{ noteIndex: number; note: MusicXmlNote }> = [];
  ...
  if (current.length) words.push({ events: current, endIndex: current[current.length - 1].noteIndex });
  ...
  for (let index = 0; index < words.length; index += 1) {
    ...
    if (nextStart !== undefined) word.endIndex = nextStart - 1;
    else if (hasExtension) word.endIndex = notes.length - 1;
    else word.endIndex = lastEvent.noteIndex;
  }
  return words;
}

function expectedForStream(notes: MusicXmlNote[], defects: AuditDefect[]): ExpectedNote[] {
  ...
  for (const word of groupWords(notes)) {
    ...
    for (let syllableIndex = 0; syllableIndex < syllables.length; syllableIndex += 1) {
      ...
      for (let noteIndex = firstIndex; noteIndex <= lastIndex; noteIndex += 1) {
        let tokens: string[];
        if (firstIndex === lastIndex) tokens = syllable.phonemes;
        else if (noteIndex === firstIndex) tokens = [...split.onset, ...split.nucleus];
        else if (noteIndex === lastIndex) tokens = [...split.nucleus, ...split.coda];
        else tokens = split.nucleus;
...
function allSnapshotNotes(snapshot: SingingProjectSnapshot, trackIndex: number): Array<SnapshotNote & { groupIndex: number }> {
...
function resolveTrackIndex(part: MusicXmlPart, snapshot: SingingProjectSnapshot, trackMap: Record<string, number>): number | undefined {
  if (trackMap[part.id] !== undefined) return trackMap[part.id];
  if (trackMap[part.name] !== undefined) return trackMap[part.name];
...
function findSnapshotNote(
  candidates: Array<SnapshotNote & { groupIndex: number }>,
  expected: MusicXmlNote,
  options: AuditOptions
): (SnapshotNote & { groupIndex: number }) | undefined {
  const onset = Math.round(expected.onsetQuarter * BLICKS_PER_QUARTER);
  return candidates.find(
    (candidate) =>
      Math.abs(candidate.absoluteOnset - onset) <= options.onsetToleranceBlicks &&
      candidate.effectivePitch === expected.pitch
  );
}

export function fingerprintSnapshot(snapshot: SingingProjectSnapshot): string {
  return stableHash({
...
export function auditMusicXmlAgainstSnapshot(
  score: MusicXmlScore,
  sourceSha256: string,
  snapshot: SingingProjectSnapshot,
  options: AuditOptions
): LyricAuditResult {
  const defects: AuditDefect[] = [];
  const corrections: PlannedNoteCorrection[] = [];
  ...
  for (const part of score.parts) {
    ...
    partsChecked += 1;
    const streams = new Map<string, MusicXmlNote[]>();
    for (const note of part.notes) streams.set(streamKey(note), [...(streams.get(streamKey(note)) ?? []), note]);
