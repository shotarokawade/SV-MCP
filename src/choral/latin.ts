...
export function syllabifyLatin(word: string): string[] {
  ...
  for (let index = 0; index < normalized.length; index += 1) {
    if (!VOWELS.has(normalized[index])) continue;
...
function phonemizeSyllable(syllable: string, nextSyllable?: string): string[] {
  ...
  while (index < source.length) {
    ...
    if (rest.startsWith("ae") || rest.startsWith("oe")) {
      tokens.push("e"); index += 2; continue;
    }
    if (rest.startsWith("gn")) { tokens.push("J"); index += 2; continue; }
    if (/^sc[ei]/.test(rest)) { tokens.push("sh"); index += 2; continue; }
    if (rest.startsWith("ch")) { tokens.push("k"); index += 2; continue; }
    if (rest.startsWith("ph")) { tokens.push("f"); index += 2; continue; }
    if (rest.startsWith("th")) { tokens.push("t"); index += 2; continue; }
    if (rest.startsWith("qu")) { tokens.push("k", "U"); index += 2; continue; }
    if (source.slice(index, index + 2) === "ti" && nextSyllable && /^[aeiou]/.test(nextSyllable)) {
      tokens.push("s", "t", "i"); index += 2; continue;
    }
    const char = source[index];
    if (char === "c") tokens.push(/[eiy]/.test(next) ? "ch" : "k");
    else if (char === "g") tokens.push(/[eiy]/.test(next) ? "J" : "g");
    else if (char === "x") tokens.push("k", "s");
    else if (char === "z") tokens.push("d", "s");
    else if (char === "j") tokens.push("y");
    else if (char === "v") tokens.push("B");
    else if (char === "h") { /* Ecclesiastical h is normally silent. */ }
    else if (char === "q") tokens.push("k");
    else if (char === "y") tokens.push("i");
...
export function ecclesiasticalLatinWord(word: string): LatinSyllable[] {
...
export function splitSyllablePhonemes(tokens: string[]): { onset: string[]; nucleus: string[]; coda: string[] } {
  const firstVowel = tokens.findIndex((token) => PHONEME_VOWELS.has(token));
  if (firstVowel < 0) return { onset: tokens, nucleus: [], coda: [] };
  let lastVowel = firstVowel;
  while (lastVowel + 1 < tokens.length && PHONEME_VOWELS.has(tokens[lastVowel + 1])) lastVowel += 1;
  return {
...
export function formatDirectPhonemes(tokens: string[]): string {
  return `.${tokens.join(" ")}`;
}

export function normalizePhonemeString(value: string): string {
