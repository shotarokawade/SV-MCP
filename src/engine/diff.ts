...
export function computeNoteDiff(before: SynthVNote, after: SynthVNote): NoteFieldDiff[] {
  const diffs: NoteFieldDiff[] = [];
  const fields: Array<keyof SynthVNote> = [
    "onset",
    "duration",
    "pitch",
    "lyrics",
...
export function formatDiffSummary(diffs: NoteChangeDiff[]): string {
  const lines: string[] = [];
