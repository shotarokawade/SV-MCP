...
export function getSynthVScriptDirectories() {
    const home = os.homedir();
    const platform = os.platform();
    const candidates = [];
    if (platform === "darwin") {
        candidates.push(path.join(home, "Library", "Application Support", "Dreamtonics", "Synthesizer V Studio 2", "scripts"), path.join(home, "Library", "Application Support", "Dreamtonics", "Synthesizer V Studio", "scripts"));
    }
    else if (platform === "win32") {
        const appdata = process.env.APPDATA || path.join(home, "AppData", "Roaming");
        const userDocs = path.join(home, "Documents");
        candidates.push(path.join(appdata, "Dreamtonics", "Synthesizer V Studio 2", "scripts"), path.join(userDocs, "Dreamtonics", "Synthesizer V Studio 2", "scripts"), path.join(appdata, "Dreamtonics", "Synthesizer V Studio", "scripts"), path.join(userDocs, "Dreamtonics", "Synthesizer V Studio", "scripts"));
    }
    else {
        candidates.push(path.join(home, ".local", "share", "Dreamtonics", "Synthesizer V Studio 2", "scripts"), path.join(home, ".local", "share", "Dreamtonics", "Synthesizer V Studio", "scripts"));
    }
    return candidates;
}
...
//# sourceMappingURL=install-lua-scripts.js.map