...
export function getDefaultIpcDir() {
...
export class MailboxIPC {
    ...
    async execute(action, params = {}, customTimeoutMs) {
        await this.init();
            ...
            while (Date.now() - startTime < timeout) {
                try {
                    ...
                    if (resContent && resContent.trim()) {
                        ...
                        const response = JSON.parse(resContent);
                        if (response.id !== id) {
                            continue;
                        }
                        if (!response.success) {
    ...
    async cleanupStaleFiles(maxAgeMs = 60000) {
        const now = Date.now();
